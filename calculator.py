...
for x in range(4):
    for y in range(3):
        if count < len(operations):
            ...
            count += 1
            button.grid(row=x+2, column=y+3, padx=2)

Button(root, text="AC", width=4, height=2, command=clear_all).grid(row=5)
Button(root, text="=", width=4, height=2, command=calculate).grid(row=5, column=2)

Button(root, text="<-", width=4, height=2, command=backspace).grid(row=5, column=4)
root.mainloop()
